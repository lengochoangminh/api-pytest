...
def test_case():
    ...
    try:
        ...
        assert error_code != 0 or response.status_code in [400, 401, 403, 404, 422], (
            f"❌ Missing clientId was accepted — errorCode: {error_code}, status: {response.status_code}"
        )
        ...
        assert empty_passed == len(empty_client_id_tests), (
            f"❌ {len(empty_client_id_tests) - empty_passed} empty clientId case(s) were incorrectly accepted"
        )
        ...
        assert unknown_passed == len(unknown_client_id_tests), (
            f"❌ {len(unknown_client_id_tests) - unknown_passed} unknown clientId case(s) were incorrectly accepted"
        )
        ...
        assert malicious_passed == len(malicious_tests), (
            f"❌ {len(malicious_tests) - malicious_passed} malicious payload(s) were incorrectly accepted"
        )
